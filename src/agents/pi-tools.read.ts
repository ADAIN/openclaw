...
export function createSandboxedReadTool(params: SandboxToolParams) {
  ...
  // We combine the OpenClaw read tool wrapper AND the ignore check.
  // We need to inject the ignore check into createOpenClawReadTool logic, or wrap it.
  // The local version of createSandboxedReadTool wrapped it:
  // return wrapSandboxPathGuard(createOpenClawReadTool(base), root);
  // But now we have params object.
  // Let's use the upstream createSandboxedReadTool structure but wrap it with wrapToolWorkspaceRootGuard which does the ignore check.

  const openClawRead = createOpenClawReadTool(base, {
    modelContextWindowTokens: params.modelContextWindowTokens,
    imageSanitization: params.imageSanitization,
  });

  return wrapToolWorkspaceRootGuard(openClawRead, params.root);
}
